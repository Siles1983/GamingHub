--[[
    Gaming Hub – Snake
    UI/Snake_SettingsPanel.lua
    Version: 1.0.0 (Multilanguage via Language.lua)

    Alle sichtbaren Strings kommen aus GamingHub.GetLocaleTable("SNAKE").
]]
...
local function BuildSnakeSettingsPanel(parent)
    ...
    local previewItems = { "head", "body", "food" }
    local previewTexs  = {}

    local function UpdateThemePreview()
        local theme = T:GetTheme(S:Get("theme"))
        for i, key in ipairs(previewItems) do
            local sym = theme[key]
    ...
    CreateSimpleDropdown(cA, 0, 0, leftW - PAD*2 - 24, L["label_theme"],
        ...
        function(key)
            S:Set("theme", key)
            if GamingHub.SNK_Renderer then GamingHub.SNK_Renderer._currentTheme = key end
            UpdateThemePreview()
        end
    )
    ...
    local previewLabels = { L["preview_head"], L["preview_body"], L["preview_food"] }
    local iconY   = 118
    local iconSize= 32
    local iconGap = 16
    for i = 1, 3 do
        local bx = (i-1) * (iconSize + iconGap + 20)
        ...
        local lbl = cA:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
        lbl:SetPoint("TOP", border, "BOTTOM", 0, -2)
        lbl:SetText("|cffaaaaaa" .. previewLabels[i] .. "|r")
    end
    UpdateThemePreview()
    ...
    local boxC, cC = CreateBox(parent, L["box_guide"],
        BOX_PAD, startY + topBoxH + BOX_PAD, fullW, 155)
