--[[
    Gaming Hub
    UI/TicTacToe_SettingsPanel.lua
    Version: 1.1.0 (Multilanguage via Language.lua)

    Alle sichtbaren Strings kommen aus GamingHub.GetLocaleTable("TICTACTOE").
    Keine hartcodierten DE/EN-Texte in dieser Datei.

    Layout:
      Zeile 1 | Box A: Symbole        | Box B: Hintergrund
      Zeile 2 | Box C: Sounds (volle Breite)
]]
...
-- ============================================================
-- HILFSFUNKTION: Checkbox mit Label
-- ============================================================
...
-- ============================================================
-- HILFSFUNKTION: Dropdown mit Label
-- ============================================================
...
local function BuildTicTacToeSettingsPanel(parent)
    ...
    -- Locale-Tabelle holen (deDE oder enUS-Fallback)
    ...
    cbSymAuto:SetScript("OnClick", function(self)
        ...
        if checked then
            UIDropDownMenu_SetSelectedValue(ddSymMode, "FACTION")
        end
    ...
    cbBgFactionAuto:SetScript("OnClick", function(self)
        ...
        if checked then
            ...
            ddBgFaction:SetAlpha(1);   UIDropDownMenu_EnableDropDown(ddBgFaction)
        end
    end)
    ...
    cbBgClassAuto:SetScript("OnClick", function(self)
        ...
        if checked then
            ...
            ddBgClass:SetAlpha(1);   UIDropDownMenu_EnableDropDown(ddBgClass)
        end
    end)
    ...
    cbBgRaceAuto:SetScript("OnClick", function(self)
        ...
        if checked then
            ...
            ddBgRace:SetAlpha(1);   UIDropDownMenu_EnableDropDown(ddBgRace)
        end
    end)
    ...
    -- ============================================================
    -- BOX C: SOUNDS (volle Breite, Zeile 2)
    -- ============================================================
    ...
    cbWin:SetScript("OnClick", function(self)
        S:Set("soundOnWin", self:GetChecked())
        if self:GetChecked() then cbSoundMain:SetChecked(true) end
    end)
    cbLoss:SetScript("OnClick", function(self)
        S:Set("soundOnLoss", self:GetChecked())
        if self:GetChecked() then cbSoundMain:SetChecked(true) end
    end)
    cbDraw:SetScript("OnClick", function(self)
        S:Set("soundOnDraw", self:GetChecked())
        if self:GetChecked() then cbSoundMain:SetChecked(true) end
    end)
    ...
    resetBtn:SetScript("OnClick", function()
        ...
        BuildTicTacToeSettingsPanel(parent)
        parent:Show()
    end)
...
-- ============================================================
-- REGISTRIERUNG (identisch zur Vorgänger-Version)
-- ============================================================

GamingHub.SettingsPanel = GamingHub.SettingsPanel or {}
GamingHub.SettingsPanel._builtPanels = GamingHub.SettingsPanel._builtPanels or {}
GamingHub.SettingsPanel._builders    = GamingHub.SettingsPanel._builders    or {}

function GamingHub.SettingsPanel.RegisterBuilder(id, buildFn)
    GamingHub.SettingsPanel._builders[id] = buildFn
end

function GamingHub.SettingsPanel.OnCategoryChange(categoryID)
    local settingsFrame = _G.GamingHubUI
        and _G.GamingHubUI.GetSettingsPanel
        and _G.GamingHubUI.GetSettingsPanel()
    if not settingsFrame then return end

    local inRegistry = false
    for _, info in ipairs(GamingHub._gameRegistry or {}) do
        if info.id == categoryID then inRegistry = true; break end
    end
    if not inRegistry then return end

    local panels   = GamingHub.SettingsPanel._builtPanels
    local builders = GamingHub.SettingsPanel._builders
    ...
    if buildFn then
        if not panels[categoryID] then
            local panel = CreateFrame("Frame",
                "GamingHub_Settings_" .. categoryID, settingsFrame)
...
GamingHub.SettingsPanel.RegisterBuilder("TICTACTOE", BuildTicTacToeSettingsPanel)

local initFrame = CreateFrame("Frame", "GamingHub_TTT_SettingsInitFrame")
initFrame:RegisterEvent("PLAYER_ENTERING_WORLD")
initFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_ENTERING_WORLD" then
        self:UnregisterEvent("PLAYER_ENTERING_WORLD")
        C_Timer.After(0.1, function()
            local first = GamingHub._gameRegistry and GamingHub._gameRegistry[1]
            if first then
                GamingHub.SettingsPanel.OnCategoryChange(first.id)
            end
        end)
    end
end)
