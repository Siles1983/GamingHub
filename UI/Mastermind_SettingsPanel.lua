--[[
    ...
    UI/Mastermind_SettingsPanel.lua
    Version: 1.0.0 (Multilanguage via Language.lua)

    Alle sichtbaren Strings kommen aus GamingHub.GetLocaleTable("MASTERMIND").
]]
...
local function BuildMastermindSettingsPanel(parent)
    ...
    local themeOpts = {}
    for _, t in ipairs(themeList) do themeOpts[#themeOpts+1] = { key = t.key, label = t.name } end
    ...
    CreateSimpleDropdown(cA, 0, 0, leftW - PAD*2 - 24, L["label_theme"],
        ...
        function(key)
            S:Set("theme", key)
            if GamingHub.MM_Renderer then GamingHub.MM_Renderer.selectedTheme = key end
            UpdateThemePreview()
        end
    )
    ...
    local pegY   = iconY + iconSize + 8
    local pegLbl = cA:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    pegLbl:SetPoint("TOPLEFT", cA, "TOPLEFT", 0, -pegY)
    pegLbl:SetText(L["peg_exact"])
    local pegLbl2 = cA:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall")
    ...
    local boxC, cC = CreateBox(parent, L["box_guide"],
        BOX_PAD, startY + topBoxH + BOX_PAD, fullW, 130)
