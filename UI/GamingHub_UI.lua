...
local MAIN_H   = 640
local CAT_W    = 196
local HEADER_H = 40   -- Blizzard: Header BOTTOMLEFT y=-49 (skaliert von -38)
local TAB_H    = 32
local PAD      = 8
...
-- ============================================================
local VALID_TABS = { GAMES=true, SCOREBOARD=true, SETTINGS=true }
NexusTabState = { activeTab="GAMES", activeCategory="TICTACTOE", activeSettingsCategory="TICTACTOE" }
local tabCBs = {}
NexusTabs = {}
...
function NexusTabs.RefreshPanelVisibility()
    ...
    -- Games-Kategorie-Panel: nur im Games-Tab sichtbar
    if F.catPanel then
        if a=="GAMES" then
            F.catPanel:Show()
        else
    ...
    -- Settings-Kategorie-Panel: nur im Settings-Tab sichtbar
    if F.settingsCatPanel then
        if a=="SETTINGS" then
            F.settingsCatPanel:Show()
        else
            F.settingsCatPanel:Hide()
        end
    end

    -- Content-Ausrichtung
    -- Games-Tab und Settings-Tab haben beide ein Kategorie-Panel links
    if F.content then
        F.content:ClearAllPoints()

        if a=="GAMES" or a=="SETTINGS" then
            F.content:SetPoint("TOPLEFT", F.main, "TOPLEFT",
                CAT_W + PAD + 22, -(HEADER_H + PAD))
        else
            F.content:SetPoint("TOPLEFT", F.main, "TOPLEFT",
                PAD + 9, -(HEADER_H + PAD))
        end
    ...
    if F.contentLabelFS then
        ...
        if a=="GAMES" then
            -- zeigt den aktuell gewählten Game-Namen (Label aus Registry, nicht catID)
            local gameCatID = NexusTabState.activeCategory
            local gameLabel = gameCatID
            if GamingHub and GamingHub._gameRegistry then
                for _, info in ipairs(GamingHub._gameRegistry) do
                    if info.id == gameCatID then
                        gameLabel = info.label
                        break
                    end
                end
            end
            F.contentLabelFS:SetText(gameLabel or "Spiele")
        elseif a=="SETTINGS" then
            -- zeigt das Label des gewählten Spiels (aus Registry, nicht catID)
            local settingsCatID = NexusTabState.activeSettingsCategory
            local settingsLabel = settingsCatID
            if GamingHub and GamingHub._gameRegistry then
                for _, info in ipairs(GamingHub._gameRegistry) do
                    if info.id == settingsCatID then
                        settingsLabel = info.label
                        break
                    end
                end
            end
            F.contentLabelFS:SetText(settingsLabel or "Einstellungen")
        else
            F.contentLabelFS:SetText(labels[a] or "")
        end
    end
end
...
local function BuildMainFrame()
    ...
    mL:SetSize(16, 580); mL:SetPoint("LEFT", f, "LEFT", 14, 0)
    ...
    mR:SetSize(16, 580); mR:SetPoint("RIGHT", f, "RIGHT", -13, 0)
...
local function GetCategories()
    -- Dynamisch aus der Spiel-Registry lesen
    -- Reihenfolge = Reihenfolge der RegisterGame()-Aufrufe (= TOC-Ladereihenfolge)
    local cats = {}
    if GamingHub and GamingHub._gameRegistry then
        for _, info in ipairs(GamingHub._gameRegistry) do
            table.insert(cats, { id = info.id, label = info.label })
        end
    end
    return cats
end
...
local function BuildCategoryPanel(parent)
    ...
    cp:SetPoint("TOPLEFT", parent, "TOPLEFT", 22, -(HEADER_H + PAD))
    ...
    for i, cat in ipairs(cats) do
        ...
        btn:SetPoint("TOP", sc, "TOP", 7, -(i-1)*26)
        totalH = totalH + 26
    ...
    -- Alle Container + StopGame + richtiger Renderer: vollständig dynamisch via Registry
    if GamingHub then
        for _, info in ipairs(GamingHub._gameRegistry or {}) do
            local container = info.container and GamingHub[info.container]
            if container then container:Hide() end

            local eng = info.engine and GamingHub[info.engine]
            if eng and eng.state and eng.state ~= "IDLE" and eng.StopGame then
                eng:StopGame()
            elseif not eng then
                local rnd = info.renderer and GamingHub[info.renderer]
                if rnd and rnd.state and rnd.state ~= "IDLE" and rnd.StopGame then
                    rnd:StopGame()
                end
            end
        end
        ...
        for _, info in ipairs(GamingHub._gameRegistry or {}) do
            if info.id == self.catID then
                local container = info.container and GamingHub[info.container]
                if container then container:Show() end
                local rnd = info.renderer and GamingHub[info.renderer]
                if rnd and rnd.EnterIdleState then rnd:EnterIdleState() end
                break
            end
        end
    end
    ...
    if fpr and fpr.OnCategoryChange then 
        fpr:OnCategoryChange(self.catID) 
    end
end)
...
-- ============================================================
-- SETTINGS-KATEGORIE-PANEL
-- Exakte Kopie des Games-Kategorie-Panels.
-- Zeigt dieselben Spiele – jedes Spiel hat eigene Einstellungen.
-- Nur im Settings-Tab sichtbar.
-- ============================================================
local function BuildSettingsCategoryPanel(parent)
    local cp = CreateFrame("Frame", "NexusSettingsCategoryPanel", parent, "BackdropTemplate")
    cp:SetPoint("TOPLEFT",    parent, "TOPLEFT",    22, -(HEADER_H + PAD))
    cp:SetPoint("BOTTOMLEFT", parent, "BOTTOMLEFT",  9,  TAB_H + PAD + 2)
    cp:SetWidth(CAT_W)

    -- Backdrop: identisch zum Games-Kategorie-Panel
    cp:SetBackdrop({
        bgFile   = nil,
        edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
        tile     = true, tileEdge = true, edgeSize = 16,
    ...
    -- ScrollFrame
    local sf = CreateFrame("ScrollFrame", "NexusSettingsCatScroll", cp, "UIPanelScrollFrameTemplate")
    sf:SetPoint("TOPLEFT",     cp, "TOPLEFT",      5,  -8)
    sf:SetPoint("BOTTOMRIGHT", cp, "BOTTOMRIGHT", -22,  6)
    ...
    for i, cat in ipairs(cats) do
        local btn = CreateFrame("Button", "NexusSettingsCategoryBtn_" .. cat.id, sc)
        btn:SetSize(CAT_W - 30, 24)
        btn:SetPoint("TOP", sc, "TOP", 7, -(i-1)*26)
        totalH = totalH + 26
        ...
        local function Refresh()
            local active = NexusTabState.activeSettingsCategory == btn.catID
            if active then
                btn.activeBG:SetAlpha(1.0)
                btn.accent:SetVertexColor(1.00, 0.82, 0.00, 1)
                btn.lbl:SetTextColor(1.00, 1.00, 1.00)
        ...
        btn:SetScript("OnEnter", function(self)
            if NexusTooltip_Show then
                NexusTooltip_Show(self, nil, "TOOLTIP_SETTINGS_CATEGORY_" .. self.catID .. "_BODY")
            end
        end)
        btn:SetScript("OnLeave", function(self)
            if NexusTooltip_Hide then NexusTooltip_Hide() end
        ...
        btn:SetScript("OnClick", function(self)
            NexusTabState.activeSettingsCategory = self.catID
            for _, b in ipairs(F.settingsCatBtns) do b.Refresh() end
            NexusTabs.RefreshPanelVisibility()

            -- Settings-Content informieren (wird von außen registriert)
            if GamingHub and GamingHub.SettingsPanel and GamingHub.SettingsPanel.OnCategoryChange then
                GamingHub.SettingsPanel.OnCategoryChange(self.catID)
            end
        end)

        table.insert(F.settingsCatBtns, btn)
    end

    sc:SetHeight(math.max(totalH, 1))
    if F.settingsCatBtns[1] then F.settingsCatBtns[1].Refresh() end

    cp:Hide() -- Standardmäßig versteckt, RefreshPanelVisibility steuert Sichtbarkeit
    F.settingsCatPanel = cp
    return cp
end
local function BuildContentPanel(parent)
    local cp = CreateFrame("Frame", "NexusContentPanel", parent, "BackdropTemplate")
    F.content = cp
	cp:SetBackdrop({
    bgFile   = nil,
    edgeFile = "Interface\\Tooltips\\UI-Tooltip-Border",
    ...
    F.games      = MakePanel("GamingHubGamesPanel")
    F.scoreboard = MakePanel("GamingHubScorePanel")
    F.settings   = MakePanel("GamingHubSettingsPanel")
    return cp
end
...
local function GetTabs()
    return {
        ...
        {id="SCOREBOARD", label="Bestenliste"},
...
local function BuildBottomTabs(parent)
    ...
    for i, tab in ipairs(tabs) do
        ...
        btn:SetPoint("TOPLEFT", parent, "BOTTOMLEFT", (i-1)*(tabW-12)+25, 15)
        btn.tabID = tab.id
...
local function Init()
    ...
    BuildSettingsCategoryPanel(main)
...
function CreateMinimapButton()
    ...
    if _G.GamingHubMinimapButton then return end

    local btn = CreateFrame("Button", "GamingHubMinimapButton", Minimap)
    btn:SetSize(32, 32)
	...
	-- GamingHub Renderer initialisieren
	-- GamingHub Renderer initialisieren (dynamisch via Registry)
if _G.GamingHub and _G.GamingHub._gameRegistry then
    local firstID = _G.GamingHub._gameRegistry[1] and _G.GamingHub._gameRegistry[1].id
    for _, info in ipairs(_G.GamingHub._gameRegistry) do
        local rnd = info.renderer and _G.GamingHub[info.renderer]
        if rnd and rnd.Init then
            rnd:Init()
        end
        local container = info.container and _G.GamingHub[info.container]
        if container then
            -- Erstes Spiel anzeigen, alle anderen verstecken
